...
! function (t, o) {
    ...
    return o.defaults = {
        ...
        buildToast: function () {
            ...
            var destinationC = document.createElement('a');

            t.appendChild(tG);
            t.appendChild(tC);
            t.appendChild(tP);
            tG.className = 'toastify-msg';
            tC.className = 'toastify-btn';
            tP.className = 'toastify-progress';

            tP.style = "transition: all "+this.options.duration+"ms linear;"
            setTimeout(()=>{
                tP.style.width = "100%"
            }, 100);
            ...
            t.addEventListener("mouseover", (function (o) {
                tP.style.width = tP.offsetWidth+"px";
            ...
            else if (this.options.escapeMarkup ? tG.innerHTML = this.options.text : tG.innerHTML = this.options.text, "" !== this.options.avatar) {
                var s = document.createElement("div");
                t.style.color = '#fff'
                s.innerHTML = this.options.avatar, s.className = "toastify-avatar", "left" == this.options.position || !0 === this.options.positionLeft ? t.appendChild(s) : t.insertAdjacentElement("afterbegin", s)
                s.style.color = this.options.style.color;
            }
            if (!0 === this.options.close) {
                var e = document.createElement("span");
                e.innerHTML = "Fechar", e.className = "toast-close", e.addEventListener("click", function (t) {
                ...
                ("left" == this.options.position || !0 === this.options.positionLeft) && n > 360 ? t.insertAdjacentElement("afterbegin", e) : tC.appendChild(e)

                if(this.options.destination !== undefined){
                    e.style = "min-height: 30px;"
                }else{
                    e.style = "min-height: 45px;"
                }
            }
            ...
            if(this.options.destination !== undefined){
                tC.appendChild(destinationC)

                destinationC.className = 'action-toastify';
                destinationC.href = this.options.destination;
                destinationC.target = '_blank';
                destinationC.innerText = this.options.destinationMsg;

                if(!0 !== this.options.close){
                    destinationC.style = "min-height: 45px; border: 0;"
                }else{
                    destinationC.style = "min-height: 30px;"
                }

            }
            if (void 0 !== this.options.destination && destinationC.addEventListener("click", function (t) {
                ...
                }.bind(this)), "function" == typeof this.options.onClick && void 0 === this.options.destination && destinationC.addEventListener("click", function (t) {
