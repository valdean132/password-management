...
(function ($) {
    ...
    $.fn.aksFileUpload = function (options) {
        ...
        var settings = $.extend({
                ...
                label: "Arraste e solte seus arquivos ou navegue"
            },
            options
        );
        var customInput = settings.input.replace("#", "");
        storedFiles[customInput] = [];
        var storedFilesName = storedFiles[customInput];
        ...
        return this.each(function (i) {
            ...
            const html =
                ...
                '" id="' +
                customInput +
                '"not-null required permission_alter="1" hidden ' +
                aksMultiplte +
                " /></div>";
            aks.append(html);
            ...
            function label() {
                ...
                var labelhtml =
                    '<label for="' +
                    customInput +
                    '" class="aks-file-upload-label"><i class="bi bi-cloud-arrow-up-fill"></i><span>' +
                    settings.label +
                    "</span></label> <div class='aks-file-upload-contain' id='touchFlow-"+customInput+"'><ul></ul></div>";
            ...
            function file(file) {
                ...
                function allOkey() {
                    ...
                    if (!file.type.match("image.*")) {
                        if(settings.multiple === false){
                            var elementFile = $(`[file-upload="${settings.fileUpload.replace("#", "")}"] [data-file]`);
                            
                            if(elementFile.length >= 1){

                                var delElement = elementFile.attr('data-file');
                                $(elementFile).animate({
                                    height: 0
                                }, 200);
                                setTimeout(()=>{
                                    $(delElement).fadeIn(650);
                                    $(delElement).remove();
                                }, 500);
                                dataRemove(delElement);
                            }
                        }
                        var filehtml =
                            '<li data-file="' +
                            ...
                            '" data-file-type="' +
                            fileTypeExtension +
                            '" class="aks-file-upload-preview aks-no-preview carouselTicker__item">' +
                            uploadbars +
                            ...
                            '<span class="aks-file-upload-title" title="'+fileName+'">' +
                            fileName +
                            "</span>" +
                            '<span class="aks-file-upload-size" title="'+fileSize+'">' +
                            fileSize +
                            "</span>" +
                            "</div>" +
                            niceButtons +
                            "</header></li>";
                        var filepreview = aks.find(".aks-file-upload-contain > ul");
                        $(filepreview).append(filehtml);
                        fileDelete();
                        $('[data-file="' + fileName + '"]').animate({
                            height: 100
                        }, 200);
                        ...
                        reader.onload = function (e) {
                            ...
                            var imagehtml =
                                ...
                                '" data-file-type="' +
                                fileTypeExtension +
                                '" class="aks-file-upload-preview carouselTicker__item">' +
                                uploadbars +
                                '<header class="aks-file-upload-p-header"><div class="aks-file-upload-p-header-icon">' +
                                fileIcon +
                                '</div><div class="aks-file-upload-p-header-content">' +
                                ...
                                fileTypeAdd +
                                '" /></div></main></li>';
                            var preview = aks.find(".aks-file-upload-contain > ul");
                ...
                if (Object.size(dataDifferent) > settings.maxFile) {
                    ...
                    error(settings.fileTypeError + fileName);
                    dataRemove(fileName);
            ...
            function fileUpload(files) {
                ...
                // var containerFiles = aks.find(".aks-file-upload-contain > div");
                // containerFiles.css('min-width', `${containerFiles.width()+(120*filesArr.length)}px`);

                filesArr.forEach(function (f) {
                    if(storedFilesName.length <= 0){
                        // storedFilesName[idFile] = f; // antigo modelo que era usado para adcionar storedFilesName.push(f);
                        storedFilesName.push(f);
                        file(f);
                    }else{
                        if(storedFilesName.find(element => element.name === f.name) === undefined){
                            // storedFilesName = f; // antigo modelo que era usado para adcionar storedFilesName.push(f);
                            storedFilesName.push(f);
                            file(f);
                        }else{
                            error(`Arquivo ${f.name} já foi selecionado...`);
                        }
                    }
                });
            ...
            function fileDelete() {
                $("[name='"+customInput+"'] .aks-file-upload-delete").on("click", function () {
                    var deleteBtn = $(this).data("delete");
                    var deleteElemet = $(this).parent().parent('[data-file="' + deleteBtn + '"]');
                    $(deleteElemet).animate({
                        height: 0
                    }, 200);
                    $(deleteElemet).fadeIn(650);
                    setTimeout(()=>{
                        // let previewDelete = deleteElemet.parent(".aks-file-upload-contain > div");
                        // previewDelete.css('min-width', `${previewDelete.width()-120}px`);
                        $(deleteElemet).remove();
                    }, 500);
                    for(i = 0; i < storedFiles[customInput].length; i++){
                        if(storedFilesName[i].name == deleteBtn){
                            storedFilesName.splice(i, 1);
                            break;
                        }
                    }


                    $("#touchFlow-"+customInput).touchFlow({
            ...
            if (settings.dragDrop === true) {
                var dropbox = aks.find(".aks-file-upload .aks-file-upload-label");
                dropbox.on("dragenter", function (ev) {
                    dropbox.addClass("drop-active");
                    dropbox.find('span').text('Solte o Arquivo');
                });
                ...
                dropbox.on("dragleave", function (ev) {
                    ...
                    dropbox.find('span').text(settings.label);
                });
