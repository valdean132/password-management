const loginRegister = () => {
            ...
            success: function(data){
                if(page == 'login-user'){
                    ...
                    if(data.type == 'error'){ // Tratando mensagens de erros
                        ...
                        let urlAtual = separaUrl($(location).attr('href'));
                        $(this).unbind(callFunctions);
    ...
    function rememberUserF(){
        ...
        let inputAcao = $('form[data-acao="login"] button[name="login-not"]');
    ...
    function rotateRegisterLogin(){
        ...
        checkedBtnLabel.click(()=>{
            ...
            let page = url.split(include_path)[1];

            let pageLogin = page.split('?').length >= 2 ? '?'+page.split('?')[1] : '';

            if(!checkedBtn.prop('checked')){
                boxMain.addClass(active)
                titleMain('Cadastre-se Agora');

                window.history.pushState('', '', include_path+'register'+pageLogin);
            }else{
                boxMain.removeClass(active);
                titleMain('Faça Login');

                window.history.pushState('', '', include_path+'login'+pageLogin);
            }
        });
    ...
    function viewsMSGInput(){
        ...
        boxMainInput.click(function(e){
            ...
            if(e.target.className == 'icon-input-atention' || e.target.className == 'icones-aviso' || e.target.className == 'bi bi-exclamation-triangle-fill'){
    ...
    function validFormRegister(){
        ...
        btnSubmitEnv.click(function(){ // botão para enviar o formulário
            $('form[data-acao="register"] [name="key"]').val(generateUUID()); // Gerando Chave de Acesso para cada usuário.

            let inputs = $('form[data-acao="register"] [permission_alter="1"]'); // Pegando os inputs de cadastro
